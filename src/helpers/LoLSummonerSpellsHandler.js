const spells = [
    {
        ...
        modes: [
          ...
          'CLASSIC',       'ARAM',
          'DOOMBOTSTEEMO', 'ULTBOOK',
          'ONEFORALL',     'ARSR',
          'ASSASSINATE',   'FIRSTBLOOD',
          'PROJECT',       'STARGUARDIAN'
        ],
        costType: 'No Cost',
        maxammo: '-1',
        range: [ 1200 ],
      ...
      },
      {
        id: 'SummonerTeleport',
        name: 'Teleport',
        description: 'After channeling for 4 seconds, teleports your champion to target allied structure.<br><br>Upgrades to Unleashed Teleport at 14 minutes.',
      ...
      },
      {
        ...
        tooltip: 'Will be replaced by your Ultimate Summoner Spell.<br /><br />Gain Attack-Smite. Attack-Smite will execute allied Buff Monsters, Epic Monsters, and Scuttle Crabs when you attack them.<br /><br /><attention>Attack-Smite does not have a cooldown.</attention>',
        maxrank: 1,
        cooldown: [ 0 ],
        cooldownBurn: '0',
        cost: [ 0 ],
...
const  findSpellImageLink = (spellId) => {
    let spellImageLink = 'FADAS';

    spells.map(spell => {
        const { key, image } = spell;
    
        if(spellId.toString() === key) spellImageLink = image.full;
    });

    return spellImageLink;
};

export { findSpellImageLink };